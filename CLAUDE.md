...
- `src/integrations/lovable/index.ts` — broker de OAuth (`@lovable.dev/cloud-auth-js`); si se decide bypasearlo para Google en dominio propio, el cambio va en `auth.tsx` (llamada directa a `supabase.auth.signInWithOAuth()`), no en este archivo
...
- **Auth**: email/contraseña funcional y verificado en ambos dominios (Vercel/getaurafarmapp.com y Lovable/aura-sync-playground.lovable.app). Google vía broker propio de Lovable (@lovable.dev/cloud-auth-js) — solo soporta google/apple/microsoft/lovable, NUNCA GitHub, y **solo funciona en dominios servidos por la infraestructura de hosting de Lovable**: falla con 404 en `/~oauth/initiate` en cualquier dominio externo (Vercel incluido), incluso con los Redirect URLs de Supabase correctamente configurados. Para que Google funcione en getaurafarmapp.com hace falta bypasear el broker: credenciales OAuth propias en Google Cloud Console + `supabase.auth.signInWithOAuth()` directo en `auth.tsx` + registrar esas credenciales en Cloud → Users → Google → "Your own credentials" del editor de Lovable, usando como Authorized redirect URI la URL nativa de Supabase (`https://ybvcomzfinoezonipbde.supabase.co/auth/v1/callback`). Pendiente de implementar.
...
- **Auth**: `src/routes/auth.tsx` — email/contraseña directo contra `supabase.auth`, Google vía `lovable.auth.signInWithOAuth` (integrations/lovable/index.ts) — pendiente de bypass para funcionar en dominio propio, ver Notas de infraestructura. `src/routes/perfil.tsx` incluye botón "Cerrar sesión" (llama a `signOut()` del store).
